...
png<-1
if(png==1)
  set_png("simon_vs_andrew_cum.png")
...
sys_data$month<-month(sys_data$start)
sys_data$hour<-hour(sys_data$start)
sys_data$date<-as.POSIXct(date(sys_data$end))
df1 <- sys_data %>% group_by(date,month) %>% summarise(gen=sum(generationEnergy/12),imps=sum(importedEnergy/12),cons=sum(consumptionEnergy/12),net=sum(net_to_grid/12))
df1$month = factor(month.abb[df1$month], levels = month.abb)
require(zoo)
df1$means<-rollmean(df1$net, 7,fill = list(NA, NULL, NA))

lims <- c(as.POSIXct(min(df1$date)),max(df1$date))

breaks<-seq.POSIXt(min(lims), max(lims), by="2 weeks")

if(png==1)
  set_png("daily_net.png")
ggplot(df1)+
  #geom_smooth(aes(date,-net),method = "lm", formula = y ~ splines::bs(x, 3), se = TRUE)+
  geom_line(aes(date,-means))+
#  scale_color_viridis("",discrete=TRUE)+
#  scale_fill_viridis(discrete=TRUE)+
  #scale_x_discrete()+
  #scale_x_discrete(limits = c(0,23))+
#  scale_y_continuous(expand=c(.05,.05))+
  ...
  labs(x="Hour",y="7 Day Rolling Mean Daily Net Generation (kWh)",
       title="Net Solar Generation (2017-18 7 day rolling average)",
       caption="Source: SolarPeople system data via Neurio API\nGraph by Andrew Leach")
if(png==1)
  dev.off()
...
ggplot(df1)+
  ...
  labs(x="Date",y="Cumulative Winter Generation Shortfal (kWh)",
