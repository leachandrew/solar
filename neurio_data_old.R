...
save(list = c("my_client_id", "my_client_secret"), file = "neurio_credentials.Rdata")
...
get_neurio<-function(start_string,end_string,time_unit,time_freq,sensor_id,token_id)
{
  ...
  api_status<<-test<-httr::VERB(verb = "GET", url = api_url, 
...
p<-ggplot(df1)+
  geom_line(aes(he,gen,colour=factor(year),group=factor(year),linetype="A"),size=1.2)+
  geom_line(aes(he,net,colour=factor(year),group=factor(year),linetype="B"),size=1.2)+
  facet_wrap(~month,nrow = 3)+
  scale_linetype_manual("",values=c("solid","dotted"),labels=c("Solar Generation","Net Consumption"))+
  ...
  theme_minimal()+theme(legend.key.width = unit(2,"cm"))+
...
png<-1
if(png==1)
  set_png("hourly_net.png")
p<-ggplot(df1)+
  geom_line(aes(he,-net,colour=factor(year),group=factor(year)),size=2)+
  facet_wrap(~month,nrow = 3)+
  scale_color_viridis("",discrete=TRUE,option = "D")+
  scale_x_continuous(expand=c(0,0),breaks=c(8,12,18,22))+
  ...
  labs(x="Time",y="Hourly Net Exports (kWh)",
       title="Daily Average Solar Net-to-Grid (2017-18 Avg)",
       caption="Source: SolarPeople system data via Neurio API\nGraph by Andrew Leach")
print(p)
if(png==1)
  dev.off()
...
png<-1
if(png==1)
  set_png("hourly_cons.png")
p<-ggplot(df1)+
  geom_line(aes(he,cons,colour=factor(year),group=factor(year)),size=2)+
  facet_wrap(~month,nrow = 3)+
  scale_color_viridis("",discrete=TRUE,option = "D")+
  scale_x_continuous(expand=c(0,0),breaks=c(8,12,18,22))+
  scale_y_continuous(expand=c(0,0),breaks=pretty_breaks())+
  ...
  labs(x="Hour Ending",y="Hourly Consumption (kWh)",
       title="Daily Average Electricity Consumption (2017-18 Avg)",
       caption="Source: SolarPeople system data via Neurio API\nGraph by Andrew Leach")
print(p)
if(png==1)
  dev.off()
