...
df1$month = factor(month.abb[df1$month], levels = c(month.abb[8:12],month.abb[1:7]))
...
sys_data$month<-month(sys_data$start)
sys_data$hour<-hour(sys_data$start)
sys_data$date<-as.Date(sys_data$start)

#sys_data$month = factor(month.abb[sys_data$month], levels = month.abb)

df1 <- sys_data %>% group_by(date) %>% summarise(gen=sum(generationEnergy),imps=sum(importedEnergy),cons=sum(consumptionEnergy),net=sum(net_to_grid))
df1<-filter(df1,year(date)==2018)
df1$month = factor(month.abb[month(df1$date)], levels = month.abb)
#df1$month = factor(month.abb[df1$month], levels = c(month.abb[8:12],month.abb[1:7]))




lims <- c(min(df1$date),max(df1$date)-days(1))
#breaks<- as.POSIXct(seq.POSIXt(min(sys_data$start),max(sys_data$start), by = "1 month") )

breaks<-seq.Date(lims[1],lims[2], by="1 week")


png<-1
if(png==1)
  png(file="daily_totals.png", width = 1400, height = 900,res=130,type='cairo')
ggplot(filter(df1,year(date)==2018))+
  geom_line(aes(date,-net/12),size=2)+
  #geom_density(aes(fill="Wind Power Generation",colour=year(Time)),alpha=0.5)+
  #stat_density(geom="line",position="identity",aes(group=Year_ID,colour=Year_ID),size=1.5)+
  scale_color_viridis("",discrete=TRUE)+
  scale_fill_viridis(discrete=TRUE)+
  ...
  labs(x="Date",y="Net Energy Deliveries (kWh)",
       title="Daily Net-to-Grid (2018)",
       caption="Source: SolarPeople system data via Neurio API\nGraph by Andrew Leach")
if(png==1)
  dev.off()
...
breaks<-seq.Date(lims[1],lims[2], by="2 weeks")


png<-1
if(png==1)
  png(file="daily_totals_all.png", width = 1400, height = 900,res=130,type='cairo')
ggplot(df1)+
  geom_line(aes(date,-net/12),size=2)+
  #geom_density(aes(fill="Wind Power Generation",colour=year(Time)),alpha=0.5)+
  #stat_density(geom="line",position="identity",aes(group=Year_ID,colour=Year_ID),size=1.5)+
  ...
  labs(x="Date",y="Net Energy Deliveries (kWh)",
       title="Daily Net-to-Grid",
       caption="Source: SolarPeople system data via Neurio API\nGraph by Andrew Leach")
if(png==1)
  dev.off()


df1<-filter(df1,date>as.Date("2017-11-01"))
sum(df1$gen)/12
sum(df1$cons)/12
...
if(png==1)#set these to only turn on if you're making PNG graphs
  dev.off()
ggsave("eclipse.png",plot=last_plot(), width = 8, height = 5)
